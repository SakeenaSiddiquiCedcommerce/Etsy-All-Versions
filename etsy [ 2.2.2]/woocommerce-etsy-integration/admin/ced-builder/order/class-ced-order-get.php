...
class Ced_Order_Get {
	...
	public function get_orders( $shopId, $is_sync = false ) {
		...
		$order_limit                = isset( $saved_global_settings_data[ $shopId ]['order_limit'] ) ? $saved_global_settings_data[ $shopId ]['order_limit'] : '';
		$params                     = array(
		...
		if ( is_array( $orders ) && ! empty( $orders ) ) {
			...
			foreach ( $orders as $order ) {
				...
				if ( ! empty( $receipt_id ) ) {
					...
					$shopDetails              = $saved_etsy_details[ $shopId ];
					$shop_id                  = $shopDetails['details']['shop_id'];
					$transactions_per_reciept = etsy_request()->get( "application/shops/{$shop_id}/receipts/{$receipt_id}/transactions", $shopId );
