	...
	public function ced_variation_details( $product_id = '', $shop_name = '', $is_sync = false ) {
		...
		foreach ( $variations as $variation ) {
			...
			foreach ( $attributes as $property_name => $property_value ) {
				...
				//$var_array[$_count] = $property_value;
				$_count ++;
				$property_id = 513;
				if(!isset($p_name_1)) {
...
// var_dump($var_att_array);
		...
		//$remaining_combinations = array_intersect($possible_combinations,$prepared_combinations);

		// if( count($remaining_combinations) > 0 ) {
		// 	foreach ($remaining_combinations as $combination) {
		// 		$property_values = array();
		// 		switch ($no_property_to_use) {
		// 			case '2':
		// 			$property_values = array(
		// 				array(
		...
		// 					'values'        => array( ucwords( strtolower( $combination[0] ) ) ),
		...
		// 					'values'        => array( ucwords( strtolower( $combination[1] ) ) ),
