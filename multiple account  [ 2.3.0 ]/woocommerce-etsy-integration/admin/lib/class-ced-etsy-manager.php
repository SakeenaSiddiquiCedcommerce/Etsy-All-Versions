...
if ( ! class_exists( 'Ced_Etsy_Manager' ) ) {
	...
	class Ced_Etsy_Manager {
		...
		public function __construct() {
			...
			add_action( 'admin_notices', array( $this, 'ced_etsy_display_notice' ) );
		}

		public function ced_etsy_display_notice() {
			$response = ced_etsy_get_notification();
			if ( isset( $response['status'] ) && '200' == $response['status'] && isset( $response['message'] ) && ! empty( $response['message'] ) ) {
				if ( isset( $_GET['page'] ) && ( 'ced_etsy' == $_GET['page'] || 'cedcommerce-integrations' == $_GET['page'] ) ) {
					return;
				}
				echo "<div class='ced_etsy_notification notice notice-warning'>";
			...
			if ( isset( $_GET['state'] ) && ! empty( $_GET['code'] ) ) {
				...
				if ( isset( $response['access_token'] ) && ! empty( $response['access_token'] ) ) {
					...
					if ( isset( $shop['results'][0] ) ) {
						...
						if ( count( $user_details ) < 3 ) {
							update_option( 'ced_etsy_details', $user_details );
						}
					}
				}

				wp_redirect( admin_url( 'admin.php?page=ced_etsy&shop_name=' . $shop_name ) );
				exit;
			}
		...
		public function ced_relatime_sync_inventory_to_etsy( $meta_id, $product_id, $meta_key, $meta_value ) {
			...
			if ( '_stock' == $meta_key || '_price' == $meta_key ) {
				...
				foreach( $shop_details as $shop_name => $s_detail ){
					 ...
					 * *******************************************
					 */
					$response = ( new \Cedcommerce\Product\Ced_Product_Update( $shop_id, $product_id ) )->ced_etsy_update_inventory( $product_id, $shop_name, true );
				}
			}
		}
