...
if ( isset( $_POST['shipping_settings'] ) ) {
	...
	if ( ! empty( $shipping_title ) && ! empty( $country_id ) ) {
		...
		$response = etsy_request()->post( $_action, array_filter( $params ), $shop_name );
		if ( isset( $response['shipping_profile_id'] ) ) {
			...
			$_error = isset( $response['error'] ) ? $response['error'] : 'Shipping profile not created';
...
$redirect_uri = admin_url( 'admin.php?page=ced_etsy&section=settings&shop_name=' . $shop_name );
			...
			<form method="post" action="">
				...
				<table class="wp-list-table widefat ced_etsy_account_configuration_fields_table">
					...
					<tbody>
						...
						<tr class="ced_etsy_shipping_profile_conditional">
							<th>
