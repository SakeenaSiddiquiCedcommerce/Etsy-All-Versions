...
class Woocommmerce_Etsy_Integration_Admin {
	...
	public function ced_etsy_submit_shipment() {
		...
		if ( $check_ajax ) {
			...
			$shop_details = get_option( 'ced_etsy_details', array() );
			foreach($shop_details as $shop_name => $s_detail ){
				if ( empty( $shop_name ) ) {
	...
	*/
	...
	public function ced_etsy_inventory_schedule_manager() {
		...
		$shop_details = get_option( 'ced_etsy_details', array() );
		foreach($shop_details as $shop_id => $s_detail ){
			if (empty($shop_id)) {
				continue;
			}
			$products_to_sync = get_option( 'ced_etsy_update_inventory_chunk_products_' . $shop_id, array() );
			...
			if ( is_array( $products_to_sync[0] ) && ! empty( $products_to_sync[0] ) ) {
				...
				update_option( 'ced_etsy_update_inventory_chunk_products_' . $shop_id, $products_to_sync );
			}
		}
	}
	...
	public function ced_etsy_auto_upload_products() {
		...
		if ( isset( $product_chunk[0] ) && is_array( $product_chunk[0] ) && ! empty( $product_chunk[0] ) ) {
			$response = $ced_etsy_product->prepare_for_upload( $product_chunk[0], $shop_name, true );
		...
		if ( $check_ajax ) {
			...
			switch ( $operation ) {
				...
				case 'update_inventory':
					if ( $already_uploaded ) {
						$response = ( new \Cedcommerce\Product\Ced_Product_Update( $shop_name, $product_id ) )->ced_etsy_update_inventory( $product_id, $shop_name );
					}
					break;
				case 'update_image':
					if ( $already_uploaded ) {
						$response = ( new \Cedcommerce\Product\Ced_Product_Update( $shop_name, $product_id ) )->ced_update_images_on_etsy( $product_id, $shop_name );
					}
					break;
				case 'unlink_product':
					if ( $already_uploaded ) {
